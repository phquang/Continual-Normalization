...
class CsvLogger:
    ...
    def write(self, args: Dict[str, Any]) -> None:
        ...
        create_if_not_exists(base_path() + "results1/" + self.setting)
        create_if_not_exists(base_path() + "results1/" + self.setting +
                             "/" + self.dataset)
        create_if_not_exists(base_path() + "results1/" + self.setting +
        ...
        write_headers = False
        path = base_path() + "results1/" + self.setting + "/" + self.dataset\
        ...
        if self.setting == 'class-il':
            create_if_not_exists(base_path() + "results1/task-il/"
                                 + self.dataset)
            create_if_not_exists(base_path() + "results1/task-il/"
                                 + self.dataset + "/" + self.model)
            ...
            write_headers = False
            path = base_path() + "results1/task-il" + "/" + self.dataset + "/"\
                   + self.model + "/mean_accs.csv"
            if not os.path.exists(path):
                write_headers = True
            with open(path, 'a') as tmp:
